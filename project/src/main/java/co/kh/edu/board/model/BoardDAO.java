...
public class BoardDAO {
	...
	private final String SELECT_SQL = "select * from board2 order by num desc";
	private final String SELECT_START_END_SQL = " select * from "
			...
			+ "from (select * from board2 order by ref desc, step asc)) where rnum>=? and rnum<=?";
	private final String SELECT_COUNT_SQL = "select count(*) as count from board2";
	private final String SELECT_MAX_NUM_SQL = "select max(num) as num from board2";
	private final String SELECT_ONE_SQL = "select * from board2 where num = ?";
	private final String SELECT_PASS_ID_CHECK_SQL = "select count(*) count from board2 where num = ? and pass = ?";
	private final String DELETE_SQL = "DELETE FROM board2 WHERE NUM = ? AND PASS = ?";
	private final String UPDATE_SQL = "update board2 set writer=?,email=?,subject=?,content=? where num=?";
	...
	public BoardVO selectBoardDB(BoardVO vo) {
		...
		try {
			...
			if (rs.next()) {
				...
				String content = rs.getString("content");
				String ip = rs.getString("ip");
				bvo = new BoardVO(num, writer, email, subject, pass, readcount, ref, step, depth, regdate, content,
						ip);
			}
		} catch (SQLException e) {
			e.printStackTrace();
