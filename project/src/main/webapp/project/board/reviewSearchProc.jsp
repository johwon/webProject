...
if(searchOpt.equals("subject")){
	vo.setSubject("%"+search+"%");
	count =bdao.searchSubjectCountDB(vo);
	if (count > 0) {
		//현재페이지 내용 10개만 가져온다
		boardList = bdao.searchSubjectDB(vo);
	}
}else{
	vo.setWriter("%"+search+"%");
	count =bdao.searchWriterCountDB(vo);
	if (count > 0) {
		//현재페이지 내용 10개만 가져온다
		boardList = bdao.searchWriterDB(vo);
	}
}
...
%>
				...
				<div align="center">
					...
					if (currentPage == i) {
					%>
					<a href="reviewList.jsp?pageNum=<%=i%>"><b>[<%=i%>]
					</b> </a>
					<%
					} else {
					%>
					<a href="reviewList.jsp?pageNum=<%=i%>">[<%=i%>]
					</a>
					<%
					}
					%>
